...
# --- 設定網頁標題 ---
st.set_page_config(page_title="PPT 重組生成器 (圖號匹配修復版)", page_icon="📑", layout="wide")
st.title("📑 PPT 重組生成器 (圖號匹配修復版)")
st.caption("修正：針對 FIG. X 格式進行超寬鬆比對，解決 US2024/0027812A1 缺圖問題。")
...
def extract_images_from_pdf(pdf_stream, target_fig_text):
    ...
    try:
        ...
        for target_number in target_numbers:
            # 建立寬鬆 Regex
            # 允許 FIG 與數字間有任意空白或點
            # 範例: FIG. 6, FIG 6, Figure 6
            search_pattern = re.compile(rf'(?:FIG|FIGURE|图|圖)[\s\.]*{re.escape(target_number)}(?![0-9])', re.IGNORECASE)
            ...
            for i, page in enumerate(doc):
                ...
                # C. 比對圖號 (直接用 Regex 搜原始文字)
                if search_pattern.search(raw_text):
